...
import { useToggleChatFavorite } from '@/hooks/use-toggle-chat-favorite'
import { useChatFavoriteStatus } from '@/hooks/use-chat-favorites'
import { useHapticFeedback } from '@/hooks/use-haptic-feedback'

interface ChatFavoriteButtonProps {
  chatId: number
...
export function ChatFavoriteButton({ 
  ...
  const toggleFavorite = useToggleChatFavorite()
  
  // Получаем актуальный статус избранного
  const { data: favoriteStatus } = useChatFavoriteStatus(chatId)
  ...
  const handleToggle = async () => {
    try {
      ...
      await toggleFavorite.mutateAsync(chatId)
    ...
    return (
      <button
        ...
        className={`
          ...
          ${className}
        `}
        title={isFavorite ? 'Убрать чат из избранного' : 'Добавить чат в избранное'}
      >
        {isLoading ? (
